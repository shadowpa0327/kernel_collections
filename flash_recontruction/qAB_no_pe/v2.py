...
#NOTE: This version use for loop to run different block_size_d instead of unrolling it like v1.
@triton.autotune(
    configs=get_configs(),
...
def _qAB_fwd_v3(
    ...
    size_hg = tl.num_programs(axis=1)
    size_l = tl.num_programs(axis=2)

    #pid_hg, pid_l = tl.swizzle2d(pid_hg, pid_l, size_hg, size_l, 8)
    ...
    out = tl.zeros((16, BLOCK_SIZE_L), dtype=tl.float32)
    # Fix BLOCK_SIZE_D = 64, and head_dim = 128
    for j in tl.static_range(2):
        ab = tl.zeros((BLOCK_SIZE_L, BLOCK_SIZE_D), dtype=tl.float32)

        for k in tl.range(tl.cdiv(RANK, BLOCK_SIZE_R)):
            b = tl.load(B_ptrs + k * BLOCK_SIZE_R * stride_B_r + j * BLOCK_SIZE_D * stride_B_d)
            a = tl.load(A_ptrs + k * BLOCK_SIZE_R * stride_A_r, mask=offs_ls[:, None] < KV_LEN, other=0.0)
            ab = tl.dot(a, b, ab)

        ab = ab.to(tl.float16)
        q = tl.load(Q_ptrs + j * BLOCK_SIZE_D * stride_q_d, mask=offs_qls[:, None] < Q_LEN, other=0.0)
        out = tl.dot(q, ab.T, out)

    tl.store(O_ptrs, out.to(tl.float16), mask=((offs_qls[:, None] < Q_LEN) & (offs_ls[None, :] < KV_LEN)))

def qAB_v3(q: torch.Tensor, Ak: torch.Tensor, Bk: torch.Tensor) -> torch.Tensor:
    """
    ...
    grid = lambda META: (bsz, num_kv_heads // 1, triton.cdiv(kv_len, META["BLOCK_SIZE_L"]))
