...
def _qAB_fwd(
    ...
    for _ in range(0, tl.cdiv(RANK, BLOCK_SIZE_R)):
        ...
        a = tl.load(A_ptrs, mask=offs_ls[:, None] < KV_LEN, other=0.0, eviction_policy="evict_last")
    ...
    out_0 = tl.dot(q1, ab_0.trans(1, 0))
    out_1 = tl.dot(q2, ab_1.trans(1, 0))
    out = out_0 + out_1
...
def run_qk_qab_benchmark(
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            x_vals=[2**i for i in range(12, 18)],  # Testing powers of 2 from 4K to 128K
...
if __name__ == "__main__":
    ...
    Ak = torch.randn(bsz, kv_len, rank).half().cuda()
    Bk = torch.randn(bsz, num_kv_heads, rank, head_dim).half().cuda()
