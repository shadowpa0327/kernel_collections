...
from flash_recontruction import qAB_rope_v1
...
def group_query_attention_factorized_v_only(
    ...
    # 3) Compute attention weights = Q * K^T / sqrt(head_dim)
    with record_function("q@K"):
        # Compute Q @ key^T -> (bsz, num_kv_heads, group_size*q_len, kv_len)
...
def group_query_attention_factorized_RoPE(
    ...
    # 3) Compute attention weights = Q * K^T / sqrt(head_dim)
    with record_function("q@K"):
        attn_weights = qAB_rope_v1(query, key_A, key_B, theta=10000000.0)
