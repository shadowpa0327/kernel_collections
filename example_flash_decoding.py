...
def flashattn(batch, heads, groups, seqlen_kv, dim, tune=False):
    ...
    def kernel_func(block_N, block_H, num_split, num_stages, threads):
        ...
        print(f"valid_block_H: {valid_block_H}")
        @T.macro
        def flash_attn(
...
def ref_program_fa2(query, key, value, mask, glse, Output_partial):
    return flash_attn_with_kvcache(query.unsqueeze(1), key, value, causal=True)
...
if __name__ == "__main__":
    ...
    if (not args.tune):
        ...
        latency = profiler.do_bench(ref_program_fa2, warmup=500)
